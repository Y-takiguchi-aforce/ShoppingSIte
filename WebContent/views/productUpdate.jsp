...
<form action="ProductUpdate" method="post" name="myform">
	...
	<p>・商品名<br>
		<input type="text" name="p_name" max-length="128" value="<%=pb.getPname() %>" required>
	<br></p>
	
	<p>・画像<br>
		<input type="file" name="image" value="<%=pb.getImage()%>">
		now:<input type="hidden" name="imageprev" value="<%=pb.getImage()%>"><%=pb.getImage()%>
		
	<br></p>
	
	<p>・値段（税抜）<br>
		<input type="number" name="price" min="0" max-length="10" value="<%=pb.getPrice() %>" required>
	<br></p>
	
	<p>・在庫数<br>
		<input type="number" name="stock" min="0" max-length="10" value="<%=pb.getStock() %>" required>
	<br></p>
...
document.myform.update.addEventListener('click', function(){
	var result = window.confirm('更新してよろしいですか？');
